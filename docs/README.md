...
| [Benchmark-Plan.md](Benchmark-Plan.md) | **The release-gate benchmark plan.** Checkpoint list only, staged B0–B9: the competitor roster and the reason behind every inclusion and exclusion, the capability tiers that keep a comparison like-for-like, the data corpus, the workloads, the fairness rules, the reporting artifacts and the baseline policy. The method lives in the `viper_bencher` skill. | Current — realigned; B0 open, nothing measured yet |
