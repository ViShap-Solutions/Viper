...
- **7.1** [ПОДТВЕРЖДЕНО] Стабильный релиз: только `vX.Y.Z` без ведущих нулей, тег ровно на HEAD
  `origin/main`.
- **7.2** [ПОДТВЕРЖДЕНО, уточнено 2026-09-26] Pre-release: только `vX.Y.Z-alpha.N`, `-beta.N`, `-rc.N`,
  `N ≥ 1`, на коммите ветки `release/*` в origin — CD отвергает тег на любой другой ветке.
...
- **9.22** [ПОДТВЕРЖДЕНО] Ветки и теги рефакторинга (вариант c). Полный порядок работы проекта —
  `internal/Development-Workflow.md`.

```text
подготовка   benchmark/release-v1-closing сливается в main; от main создаётся release/v1.0.0
этап         rework/rN-<суть> от release/v1.0.0 → PR в release/v1.0.0 (merge-коммит); CI на PR
до R6        никаких публикаций на NuGet: формат ещё меняется
после R6     v1.0.0-beta.N на release/v1.0.0 — формат окончательный
после R9     v1.0.0-rc.N на release/v1.0.0 — только исправления
выпуск       PR release/v1.0.0 → main (merge-коммит), тег v1.0.0 на HEAD main
замер R0     локальный тег pre-rework, не пушится
ci.yml       запускается на PR и push в main, release/**, support/**
```
