...
public sealed record DeserializationLimits
{
    ...
    {
        if (MaxDepth <= 0)
            throw new BinaryTypeException($"{nameof(MaxDepth)} must be positive.");

        if (MaxArrayLength <= 0)
            throw new BinaryTypeException($"{nameof(MaxArrayLength)} must be positive.");

        if (MaxCollectionLength <= 0)
            throw new BinaryTypeException($"{nameof(MaxCollectionLength)} must be positive.");

        if (MaxDictionaryEntries <= 0)
            throw new BinaryTypeException($"{nameof(MaxDictionaryEntries)} must be positive.");

        if (MaxStringLength <= 0)
            throw new BinaryTypeException($"{nameof(MaxStringLength)} must be positive.");

        if (MaxByteBlobLength <= 0)
            throw new BinaryTypeException($"{nameof(MaxByteBlobLength)} must be positive.");

        if (MaxTotalElements <= 0)
            throw new BinaryTypeException($"{nameof(MaxTotalElements)} must be positive.");

        if (MaxMessageBytes <= 0)
            throw new BinaryTypeException($"{nameof(MaxMessageBytes)} must be positive.");
    }
}