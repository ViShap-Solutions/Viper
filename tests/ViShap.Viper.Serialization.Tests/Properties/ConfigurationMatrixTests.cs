...
public sealed class ConfigurationMatrixTests
{
    public static IEnumerable<object[]> Configurations()
    {
        ...
        yield return new object[]{"CFG-10",TestHelpers.FullPipeline(new byte[32],references:true) with { Limits = TestHelpers.TightOptions().Limits }};
    ...
    {
        var serializer=new BinarySerializer(options); var person=new Person{Name="p",Age=7,Address=new Address{City="c",Street="s"}}; var actual=serializer.Deserialize<Person>(serializer.Serialize(person)); Assert.Equal(person.Name,actual!.Name); Assert.Equal(person.Age,actual.Age);
        var list=new List<int>{1,2,3}; var actualList=serializer.Deserialize<List<int>>(serializer.Serialize(list)); Assert.Equal(list,actualList);
        if (options.Limits.MaxDepth>=2) { var node=new DeepNode{Value=1,Next=new DeepNode{Value=2}}; var actualNode=serializer.Deserialize<DeepNode>(serializer.Serialize(node)); Assert.Equal(2,actualNode!.Next!.Value); }
    }
}
