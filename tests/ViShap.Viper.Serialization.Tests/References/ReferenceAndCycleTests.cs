...
public sealed class CircularReferenceTests
{
    ...
    [Fact] public void CYC09_DeepChainBelowLimitSucceeds() { var head=new DeepNode(); var cur=head; for(int i=0;i<2;i++){cur.Next=new DeepNode();cur=cur.Next;} Assert.NotEmpty(new BinarySerializer(TestHelpers.TightOptions()).Serialize(head)); }
